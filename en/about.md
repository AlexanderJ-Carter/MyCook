---
title: About
description: About MyCook — sources, features, and credits
---

# About MyCook

**MyCook** makes home cooking easier: two beloved open-source Chinese recipe collections on one kitchen dashboard, plus search, weekly planning, and fridge matching.

**Where to go**

| URL | Role |
|-----|------|
| [cook.alexander.xin](https://cook.alexander.xin) | GitHub Pages (CDN, everyday browsing) |
| [mycook.alexander.xin](https://mycook.alexander.xin) | Self-hosted full site (includes image cookbook) |
| [cook-mcp.alexander.xin](https://cook-mcp.alexander.xin/mcp) | Remote MCP (Pocket ID / API key) |

## Why it exists

Cooking at home often means juggling several sites — one for methods, one for ingredients, one for photos. MyCook brings them together so you spend less time browsing and more time cooking.

## Content sources

| Project | What you'll find |
|---------|------------------|
| [CookLikeHOC](https://github.com/Gar-b-age/CookLikeHOC) | Home-style dishes, Lao Xiang Ji way — by stir-fry, braise, soup… |
| [HowToCook](https://github.com/Anduin2017/HowToCook) | A programmer's cooking guide, precise to the gram |
| [HowToCook Images](https://github.com/king-jingxiang/HowToCook) | 4K step photos on the full host [mycook…/howtocook-images/](https://mycook.alexander.xin/howtocook-images/) |
| [Cook Pantry](https://github.com/YunYouJun/cook) | Inspiration for “what’s in my fridge” matching |

Recipes sync automatically from upstream every day, so you're always seeing the latest.

## Highlights

| Feature | What it does |
|---------|------|
| 🔍 Search | `Ctrl+K` / `/` — 577+ recipes as you type |
| 🧊 Fridge | Pick ingredients, find matching dishes |
| 📅 Week plan | Plan Mon–Sun, stored only in your browser |
| 📚 Tips | Blanching, oil temperature, microwave know-how |
| ❤️ Toolbar | Save · Timer · Share · Print · **Copy for AI** |
| 🤖 Copy for AI | Ask ChatGPT / Claude about any recipe, zero setup |

## Copyright

MyCook is an independent curation site maintained by [alexander.xin](https://alexander.xin), not affiliated with the official projects above. **Recipes belong to their original authors and contributors.** The site's source code is open under [MIT](https://github.com/AlexanderJ-Carter/MyCook).

Stay safe around fire and electricity. Enjoy every meal.

## Feedback

[GitHub Issues](https://github.com/AlexanderJ-Carter/MyCook/issues) · Star it if you like it ⭐
